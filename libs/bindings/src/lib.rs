...
//!
//! ## Safety
//! This library wraps the internal C calls in a singleton ([Speaker]) to keep the mutable global
//! state safe. In this future this may be changed to use the asynchronous features of `espeakNG`
//! however I currently don't trust it to be safe without a global lock.
//!
...
impl Speaker {
    ...
    fn initialise(voice_path: Option<&str>) -> Result<Self> {
        ...
        //         // Loop through this C event until the terminate event, as this contains the
        ...
        unsafe {
            ...
            bindings::espeak_ng_InitializePath(
                voice_path.map_or(std::ptr::null(), |path| path.as_ptr()),
            );
        ...
        self_.set_voice_raw(Self::DEFAULT_VOICE)?;
        Ok(self_)
    }
    ...
    fn _synthesize(&mut self, text: &str, user_data: Option<&AudioBuffer>) -> Result<()> {
        ...
        // Wait until TTS has finished being generated, could be made concurrent but global
        // state....
    ...
    ///
    /// # Errors
    /// If [`PhonemeGenOptions::Mbrola`] or [`PhonemeGenOptions::MbrolaFile`] is passed, internal C
    /// calls may fail.
    ...
    fn text_to_phonemes_mbrola(
        ...
        let raw_file_fd = match file {
            ...
            None => tempfile::tempfile()?.into_raw_fd(),
        };
