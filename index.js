...
switch (platform) {
  ...
  case 'win32':
    ...
    break
  case 'darwin':
    localFileExisted = existsSync(join(__dirname, 'espeak-rs.darwin-universal.node'))
    try {
      if (localFileExisted) {
        nativeBinding = require('./espeak-rs.darwin-universal.node')
      } else {
        nativeBinding = require('@speechifyinc/espeak-rs-darwin-universal')
      }
      break
    } catch {}
    switch (arch) {
